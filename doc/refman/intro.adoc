...
This notation can also be used to ease the writing of string which contains weird characters:

```
#<<END
x= \\frac{-b \\pm \\sqrt{b^2 - 4ac}}{2a}  \"See Fig. 2\"
END
```
...
NOTE: {{stklos}} also implements **raw strings** as defined in {{link-srfi
267}}. The purpose of raw strings is identical of STklos *here strings*. For
instance, the previous here string could be written as +
`#""x= \\frac{-b \\pm \\sqrt{b^2 - 4ac}}{2a}   "See Fig. 2"""`
