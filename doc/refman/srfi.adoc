...
=== Using a SRFI
Importing a SRFI is the simplest way to use it. For instance, to use
{{quick-link-srfi 1}}, one can just do:
```scheme
(import (srfi 1))
```
This will

1. eventually load some code (if the SRFI is an external SRFI — see below)
2. import the exported symbols of `(srfi 1)` in the current module.

Another way to use a SRFI can be done with the special form
`cond-expand` defined in {{quick-link-srfi 0}} which is fully
supported by {{stklos}}. This form accepts features identifiers which
are of the form *_srfi-n_* where *_n_* represents the number of the
SRFI supported by the implementation (for instance *_srfi-1_* or
*_srfi-30_*).

For instance, to use {{link-srfi 1}}, you can use
```scheme
(cond-expand
 (srfi-1))
```

{{stklos}} also offers the (deprecated) primitive *require-feature*
which ensures (eventually) the loading of files needed to use a given
SRFI. This primitive accepts several forms to ensure that the SRFI can
be used.  For instance, to use {{quick-link-srfi 1}}, the following forms
are possible:
...
with *import*, *cond-expand* or *require-feature*, as described above.


*List of external SRFIs:*
{{for-each (lambda (x)
             (let ((nb (car x)))
               (unless (srfi-embedded? nb)
                 (printf "``srfi-~a`` " nb))))
            *srfi-database*}}
...
For some SRFIs, a feature identifier is associated (see the list of defined
feature identifiers in <<feature_identifiers>>). These SRFIs can be used by
using their feature identifier, rather than their number.

For instance, to use {{quick-link-srfi 1}}, whose associated feature identifier is `lists`,
one can use one of the following forms:

```scheme
(import (srfi lists))    ;; equivalent to (import (srfi 1))
(cond-expand (lists))
(require-feature 'lists) ;; or "lists"
```
...
{{quick-link-srfi 261}} is fully supported. It defines some named feature for existing
SRFIs. Most of the names used in this SRFI are reused in {{stklos}}.
...
{{quick-link-srfi 267}} is fully supported. It defines a syntax for raw strings which are similar
to standard {{stklos}} *here strings*.
