...
To access, to the functions and conditions of these SRFI, you need
to import both libraries `(srfi 35)` and `(srfi 36)`:

```scheme
(import (srfi 35) (srfi 36))
```

However, a quicker (and less portable) way the access to these symbols
can be to require the file `"conditions"`:
